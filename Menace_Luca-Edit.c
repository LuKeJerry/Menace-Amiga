...
What follows is ta reconstruction of the Amiga game Menace, originally coded in
68000 assembler by David Jones of DMA Design, and published by Psygnosis in 
1988. This version was coded by Jimmy Maher in 2010, and may be freely 
distributed.
...
This code is best understood in tandem with Chapter 8 of my book 
"The Future Was Here: The Commodore Amiga," which describes its techniques at
a much higher level of abstraction than the comments which follow in the source
 proper.
*/

#include <exec/types.h>
#include <exec/memory.h>
#include <proto/dos.h> // LUCA: added proto/
...
#define BytesPerBackPlane (BackgroundWidth*ScreenHeight) // 100 * 192 = 19200
#define BytesPerForePlane (ForegroundWidth*ScreenHeight) // 92 * 192 = 17664
#define BackgroundMemory (2*NumberPlanes*BytesPerBackPlane) // 2 * 3 * 19200 = 115200
#define ForegroundMemory (NumberPlanes*BytesPerForePlane) // 3 * 17664 = 52992
#define MemNeeded (BackgroundMemory+ForegroundMemory) // 115200 + 52992 = 168192
// #define MemNeeded 204800 // 0x32000 = 204800 bytes
...
// struct DosLibrary *DOSBase; // LUCA: removed to fix "Global symbol _DOSBase"
...
UWORD __chip copperlist[] = {
   0xa01,0xff00, // start copperlist - wait (0xFF00) for line 0xA = 10, row 01
   bplpt,0,bplpt+2,0,
   ...
   bplcon0,0x6600,                   //  BPLCON0 ($dff100) = 1100111001000 
                                     //   = Dual PlayField 3+3 plane Lowres
   /*scroll.value */
   bplcon1,0xff, 
   bpl1mod,0x36,
   bpl2mod,0x2e,
   ...
   0xdf01,0xff00, // wait (0xFF00) [ignoring horizontal counters] for line 0xdf = 223, row 01
   // 0xc001,0xff00, // what if we wait line 192 (0xc0) here? -- nothing good happens, 
                     //  or actually: the bottom panel is shifted 31 pixel up, covering the lower part of the tunnel screen
   bplcon1,0x0,
   bplcon0,0x4200,
   ddfstrt,0x30,
   ...
   0xffff,0xfffe // WAIT (0xfffe) for line FF = 255
                 // End-Of-Copperlist
...
struct Path {
   ...
   /*After this header comes the actual script. It consists of pairs of bytes, 
   the first representing the X value to move to next, the second the Y value. 
   X can also have special codes:
...
ULONG alien_pointers[] = {
   explosion1,0,0,guardian_eye1,explosion1,tadpole,eye,bubble,jellyfish1,
   jellyfish2,bordertl,borderbl,bordertr,borderbr,mouth,slime,snakebody,
   snakehead,0,0,0,0,0,0
};
...
BOOL setup_addresses(struct Background *screenbgnd,UWORD size,UWORD soffset,UWORD boffset,PLANEPTR *planes);
PLANEPTR *check_end_screen();
void check_path();
void update_missiles();
void kill_alien(UWORD x,UWORD y);
void explode_alien(struct Path *path);
BOOL draw_missile(UWORD x,UWORD y,struct Background *table,UWORD size,UBYTE *buffer,UBYTE *sprite,UBYTE *mask);
...
main()
{
   ...
   map=readFile("map.dat",&mapSize,0);
   panel=readFile("panel.dat",&panelSize,MEMF_CHIP);
   graphics=readFile("foregrounds.dat",&graphicsSize,MEMF_CHIP);
   ships=readFile("ships.dat",&shipsSize,MEMF_CHIP);
   paths=readFile("paths.dat",&pathsSize,0);
   aliens=readFile("aliens.dat",&aliensSize,MEMF_CHIP);
   ...
   /*Set up pointers to each of the 9 bitplanes we will need. 
    (Remember that the background is double-buffered.) */
   i=(ULONG)MemBase;
   ...
   /*Set up a pointer to the hardware registers that will let us control the 
    machine on the most fundamental level. */
   Hardware=(struct Custom *)0xdff000;

   TakeSystem();

   Hardware->bltafwm=0xffff; // Blitter first-word mask for source A
   Hardware->bltalwm=0xffff; // Blitter last-word mask for source A
                             // These registers should be set to all 1s for 
                             // fill mode or for line-drawing mode.
   
   GameInit();
   
   Hardware->cop1lc=(ULONG)copperlist; // Loads the copperlist 
   
   //LUCA: disable AGA...
   //    move.w    #0,$dff1fc        ; FMODE=0 - Disattiva l'AGA
   // Hardware->fmode=0;
   // DFF1FC=0x0000
   ULONG FMODE_CURR;
   FMODE_CURR=*(ULONG *)0xDFF1FC;
   
   ULONG *FMODE_REG=(ULONG *)0xDFF1FC;
   *FMODE_REG=0x0;
      
   
   //    move.w    #$c00,$dff106    ; BPLCON3=0xc00 - Disattiva l'AGA
   //Hardware->bplcon3=0xc00;;
   ULONG *BPLCON3=(ULONG *)0xDFF106;
   *BPLCON3=0xc00;
   
   // Get AGA chipset into ECS compatibility mode */
   //Hardware->bplcon4 = 0x11;
   ULONG *BPLCON4=(ULONG *)0xDFF10C;
   *BPLCON4=0x11;
   
   
   /*Reenable DMA. */
   // Let's try without the Blitter priority...
   //Hardware->dmacon=0x83e0; // 1000001111100000
   // .. nope! It's needed, otherwise there are graphic glitches, 
   /// ..and it doesn't help collision-systemhang on 68000
   
   Hardware->dmacon=0x87e0; // 1000011111100000
                            // 5432109876543210
                            //      |_______________ BLTPRI = 1 == Blitter DMA priority (over CPU micro)
   ...
   Hardware->clxcon=0xc30;
    ...
    // perhaps, just before the main game loop we reset the collision register?
    // YES! So the game doesn't lock on start - but only on the first collision! :)
    UWORD clxReset;
    clxReset=Hardware->clxdat;
   
   /*Here begins the main game loop. */
   do
   {
      /*Wait for the display to reach line 223, the start of the status panel 
       and end of the action area. */
       
      // STO CAZZO!!! Alla linea 223 finisce anche lo status panel!!! 
      ...
      if (vcount)
      {
         ...
         check_collision(); //<---- Here we go! This is the call that freezes!
    
         
         erase_missiles();
         update_missiles();
         drawfgnds(); // LUCA: just for test, let's disable foregrounds //LUCA2: now we wait for it...
         check_path();
      ...
      else
      {
         ...
         ///////////////////////////////////////////////////////////////////////
         // LUCA: just for test, let's disable aliens
         process_aliens();
...
{
   int x;
   // check if blitter is ready!
   if (Hardware->dmaconr&0x2000) {x=2}
   while (Hardware->dmaconr&0x4000) {x=1};
   
}

void waitline223()
{
   /*We have programmed the copper to set an interrupt flag when it reaches 
    line 223. This function (busy)waits for that to happen. */
   while (!(Hardware->intreqr&16)); // 16 = BIN 10000 = BIT #4 OF Interrupt request bits (clear or set) == "04 	COPER 	3 	Coprocessor"
   Hardware->intreq=16;
}
...
void checkpf(BOOL pf1)
{
   ...
   if (pf1){
      ...
      /*Reposition bitplane pointers and reset scroll to 15 if we have 
         scrolled 16 pixels. */
      if (pf1scroll<0)
         resetpf1();
    };
    ...
    /*If we have scrolled 16 pixels, reset the scroll values and 
      bitplane pointers. */
    if (pf2scroll<0)
       resetpf2();
...
void resetpf2()
{
   ...
   if (pf2count==0){
      ...
      /*Offset is the complement of pf2count, which we will plug into the 
        copper list. So we reset that to 0. */
      pf2offset=0;

      /*Check if we have reached the end of the level and should begin to 
       bring out the boss. (More to come here.) */
      if (level_end==1)
         level_end++;}
...
void drawfgnds()
{
   ...
   /*We draw a new column of foreground graphics into the hidden parts of the
     screen every 16 pixels, using the blitter.*/

   if (pf2scroll==0){

      /*Offset will always be 0 at this stage, letting us draw the column just 
      ...
      fgndpointer+=12;}
       ...
       if (pf2scroll==14)
          ...
          if (map[fgndpointer]!=255)
             ...
             /*Using an offset of 46 bytes (the width of the screen) lets us 
               draw the next column on just to the other side of the visible 
               screen. */
             drawfgnd(46);

          else

             /*A block number of 255 means we have reached the end of the level 
              and should bring out the boss. */
             level_end=1;
}
...
void setupblit()
{
   /*Straight copy from channel A to channel D, with no transformations. 
     See the Hardware Manual for more info.*/
   Hardware->bltcon0=0x9f0;
   Hardware->bltcon1=0x0;
   
   /*No mod for source, as data is stored sequentially. 
     Destination mod is set up to draw into a rectangle carved out of the 
     destination bitmaps.*/
...
void buildbackgnd()
{
   ...
   /*Set up pointers to 4 background bitplanes -- 2 for each screen, referred 
   to here as screens A and B. (Remember, we are double-buffering.) 
   The 3rd bitplane for each screen we will ignore for now, as it will not be 
   used for the background pattern. We skip forward 4 bytes from the start of 
   each bitplane to account for the clipping area at the edge which does not 
   concern us now.*/
   ...
   /*Find our starting point for the level map. The complete game obviously 
   contains more maps than just the 1 that is recreated here. 
   Each uses 144 bytes.*/
   table=backgroundtable+(level_number*144);

   for (y=0;y<12;y++){ /*12 blocks per column. */

      for (x=0;x<12;x++){ /*23 blocks per row. We draw 2 per inner loop, 
                             except the last time through. */
...
void GameInit()
{
   ...
   /*1 extra here to make the numbers line up the first time through the 
     foreground scroll. After that we will reset to 23. */
   pf2count=24;
   ...
   // LET'S TRY TO GO UP ONE PIXEL?
   // The status panel is 352x32, consuming 1408 bytes per bitplane,
   //  it starts at line 187
   //  ...so if we want to start one row below, we should add 352 pixel? = 352 bit = 44 bytes?
   // Let's try... and it works much better!!! 
   max_energy(356);
   
   // OLD JIMMY ORIGINAL ONE
   //max_energy(324-12); // (312)
   ...
   /*Load the 4 status panel bitplane addresses into the copper list. 
     The status panel is 352x32, consuming 1408 bytes per bitplane.*/
   for (i=0;i<4;i++){
...
void check_collision()
{
   ...
   int i;
    ...
    /*The clxdat register is cleared when we read it,
     so we preserve its value in a variable. */
    clx=Hardware->clxdat;
   
    /*Nothing to do here if no collision has occured, 
    or if the player has a force field. */
    ...
    if (clx&6)
    {
        ...
        if (ship_energy<0)
        {
          ...
          /*We need to caculate the sprite control longwords for the explosion 
            that will replace the usual ship sprites. */
          x=xpos;
          y=ypos;

          /*Vertical adjustment to convert screen coordinates into 
             hardware coordinates. */
          y+=44;

          /*Move the y value into the upper 8 bits of y. Our y values will 
            never exceed 255, so no need to worry about the high bits of y. */
          y=(y<<8);

          /*Horizontal adjustment. */
          x+=128;

          /*Get x and y into the upper 16 bits of the control longword, 
          discarding bit 0 of x and shifting the other bits over. */
          exp1=((y|(x>>1))<<16);

          /*Bit 0 of x goes into bit 0 of the longword. */
          exp1+=(x&1);

          /* Calculate vertical ending position for a 24 pixel high sprite. 
           (Sprites are always 16 pixels wide, so no need for a horizontal 
           ending position.) */
          y+=0x1800;
        ...
        {
        /*Reset ship_count. */
        //decrease_energy(324-12,ship_energy); //<----------- HERE IS THE BUG!
          
        // SO FROM max_energy we know that the offset is really 356, let's try it
        decrease_energy(356,ship_energy);
        // WHAT if we call max_energy here?
        //max_energy(356); /// <---- this hasshole works like a charm!
        }
    }
}
...
void explode_ship()
{
   ...
   /*The frames sequence of the explosion animation is stored in
     explosion_sequence. -1 indicates the end of the animation. */
   ...
   /*Work out the sprite addresses of the next frame. 
     Each frame consists of 3 non-attached sprites. */
...
void moveship()
{
   ...
   if (field_count){
      ...
      /*If force field is expired, reset the ship outline to black. 
      Otherwise strobe dark and light blue. */
      if (field_count==0)
         copperlist[77]=0;
      if (field_count&1)
         copperlist[77]=0xc;
   ...
   if (up){
      ...
      /*Sprite data is 560 bytes long. 
       We will adjust the pointer using up_down to point to 
        the correct image. */
      up_down=-560;
   ...
   if (right==0&&left==0){
      ...
      /*If ship is not being actively moved but still has intertia,
      decrease X and Y vectors. */
      if (xvector>0)
         xvector--;
      else
          if (xvector<0)
...
BOOL get_firekey()
{
    if (((*PortA&128)==0)||((*PortA&64)==0)) 
    {
        // CRAZY TEST!
        // decrease_energy(356,ship_energy);
        // PURE QUA SI PIANTA, anche se non con tutti i valori di energia,
        // tipo se perdi un po' di energia e spari qualche missile, funziona.
        // ora proviamo a mettere un valore fisso di energia = 64
        // ==> Idem o ancora peggio :(((
        return (TRUE);
...
void decrease_energy(UWORD offset,UWORD energy)
{
    ...
    // TO DEBUG...
    // use memwatch point to break when $100 is accessed (for example "w 0 100 2")  
    //int x;
    //UBYTE *BREAKER=(UBYTE *)0x100;
    //if (BREAKER==10) {x=1}
    //*BREAKER=0xFF;
    
    //Offset 324 for shields; 676 for cannons; 1028 for lasers.
    // ...STICA! L'offset per shields (energia) e' poi 356!!!
    //    vedi la call a max_energy

    // UWORD *loc2;
    
    UBYTE *loc3; // 3 is the lucky number! :)
    
    
    //LUCA TEST STUFFS ///
    ///UWORD energy2;
    //energy2 = energy/8;
    // FUZZY TEST (I'm Falling Asl33p... 
    // loc=(UWORD *)(panel+offset+energy2);
    ...
    //loc2=(UWORD *)(panel+offset+(energy/8));
    // EOF-OJC
    
    // dobbiamo aggiungere l'offset dell'energia. 
    // Nella funzione max_energy non era necessario perche' partivamo da sinistra
    // e coloravamo tutte le barrette andando verso destra.
    // L'offset di energia e' diviso per 8 - Attenzione che ogni pixel e' un bit,
    // quindi un byte di memoria del "panel" copre 8 pixel !
    // L'energia iniziale e' 128, diviso per 8 = 25... 
    // quindi andiamo ad annerire l'offset iniziale + 128 pixel ?!
    
        
    // Pero'... se iniziamo da (panel+offset) dovremmo essere a 0, non lasciare un buco a meta' ! 
   
        // IF WE DON'T CONSIDER ENERGY, IT WORKS:
        // loc=(UWORD *)(panel+offset);
        
    // Umme.. forse che la stessa variabile "loc" fa casino? cambiamola vah... 
    // ..ok, forse meglio... ma si vede che iniziamo ad annerire pizel troppo a destra! Why!?!?
    // Usando il 68020 si vede che siamo 8 pixel troppo a destra... ok, solo un BYTE!?
    // proviamo allora a togliere un byte dall'offset:
    // offset--; //<----- NO NO NO! DOBBIAMO USARE INDIRIZZI WORD-ALIGNED! 
    
    
    // NO DIVISIONS <-----------------------------------------------------------------------------------
    //loc2=(UWORD *)(panel+offset+(energy/8));
    //loc2=(UWORD *)(panel+offset+energy);
    
    // IF WE DON'T CONSIDER ENERGY, IT WORKS:
    // loc2=(UWORD *)(panel+offset);
    // ... in realta' no! solo prima funzionava...
    // ^^^^ EH SI, ma con 'offset--' stai ancora usando indirizzi dispari!
        
    // Qua usiamo un puntatore a Unsigned-Byte, cosi' non abbiamo problemi di 
    //   allineamenti
    loc3=(UBYTE *)(panel+offset+(energy>>3);
    

    //Remove yellow color from that section. 
    loc3[0]=((loc3[0]&0xaa)|((loc3[0]&0x55)<<2));
    loc3[44]=((loc3[44]&0xaa)|((loc3[44]&0x55)<<2));
    loc3[88]=((loc3[88]&0xaa)|((loc3[88]&0x55)<<2));
    
    //loc3[1]=((loc3[1]&0xaa)|((loc3[1]&0x55)<<2));
    //loc3[22]=((loc3[22]&0xaa)|((loc3[0]&0x55)<<2));
    //loc3[23]=((loc3[23]&0xaa)|((loc3[1]&0x55)<<2));
    //loc3[44]=((loc3[44]&0xaa)|((loc3[0]&0x55)<<2));
    //loc3[45]=((loc3[45]&0xaa)|((loc3[1]&0x55)<<2));
    
    
    /*
    loc2[0]= ((loc2[0] &0xaaaa)|((loc2[0]&0x5555)<<2));
    loc2[22]=((loc2[22]&0xaaaa)|((loc2[0]&0x5555)<<2));
    loc2[44]=((loc2[44]&0xaaaa)|((loc2[0]&0x5555)<<2)); // <-------------------------- THESE THREE!!!! 
    */
    
    // While I'm too tired to xplain all these &s and |s, 
    // from function max_energy I see the same array element |ed,
    // while original Jimmy code was |ing 0,22,44 against element 0 (see original code above)
    // let's try to keep same array elements...
    // AND it's much better!!! Actually it kinda works now...
    // == on 68020 the energy bars are decreased correctly... 
    // ... or perhaps it decreases 4 of them instead of 2?
    // Anyway, on 68000 now we see that 4 bars of energy are removed
    // ... then the game hangs... so... it hangs after one or two calls of this function!?
    
    /*// Questo non quaglia perche' il 68000 puo' accedere solo ad indirizzi WORD-ALIGNED (PARI!)
    loc2[0]= ((loc2[0] &0xaaaa)|((loc2[0] &0x5555)<<2));
    loc2[22]=((loc2[22]&0xaaaa)|((loc2[22]&0x5555)<<2)); // 22 * 8 = 176 pixel
    loc2[44]=((loc2[44]&0xaaaa)|((loc2[44]&0x5555)<<2)); // 44 * 8 = 352 pixel // this should be one line!
    */
 
    // QUESTO DOVREBBE RISOLVERE TUTTI I NS PROBLEMI 68000 vs 68020
    
 
    // Troppi &s and |s per un 68000? Proviamo un assegnamento grezzo?
    // NOPE! Si pianta pure questo... :-(
    /*
    loc2[0]=(loc2[0]&0xaaaa);
    loc2[22]=(loc2[22]&0xaaaa);
    loc2[44]=(loc2[44]&0xaaaa);
    */
    // PIU' GREZZO?
    /* 
    loc2[0]=0;
    ...
    */
    // Niente da fare :((( 
    // Solo qunado tutte le assegnazioni dei "loc" sono commentate, il codice
    //  gira su 68000
...
void change_ship()
{
   /*Status 0 == no additional weapons (ship 1). 
     Status 1 == cannons (ship 2). 
     Status 2 == lasers (ship 3). 
     Status 3 == cannons and lasers (ship 4). 
     We find the address of the proper ship and store it in shipaddress. */
...
void drawbobs(struct Background *screenbgnd,UBYTE *alien,UBYTE *mask)
{
   ...
   if (upsidedown){
      Hardware->bltdmod=-106;
      Hardware->bltcmod=-106;}
   else{
      Hardware->bltdmod=94;
      Hardware->bltcmod=94;};
...
void TakeSystem()
{
   ...
   /*Also kill interrupts at the CIA controller itself, which handles keyboard 
    interrupts among others, just to be on the safe side. */
   *ICRA=0x7f;

   /*Save current vectors for level 2 and 3 interrupts 
    (those dealing with keyboard and other I/O and with the video hardware), 
    so that we can replace them with new routines. */
